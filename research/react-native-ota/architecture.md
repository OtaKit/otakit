...
Date: 2026-09-11. Status: implementation plan, revised against published RN/Expo sources, executable JavaScript integration probes, and both delivery reviews. It specifies RN-only corrections to the five findings in the [deep review](deep-review.md). Implementation now includes local database migrations, shared RN publication/upload services, bare RN export, durable publication receipts, baseline byte verification, dashboard targeting and native updaters validated in iOS simulator and Android API 24/36 Release emulator builds. Broader device and Expo acceptance, completed native build hooks and collection orchestration remain unfinished; [implementation status](implementation-status.md) records the precise scope and evidence. RN creation remains unavailable and production has not been migrated. The [integration verification record](integration-verification.md) distinguishes upstream facts from implementation acceptance; [review resolution](review-resolution.md) records the delivery decisions and their checks.
...
| RN baseline provenance | Nullable typed `embeddedReceipt` or `baselineBundleId` on bundle/upload session | Identify an archived baseline or bind an OTA artifact to its uploaded baseline |

An RN baseline import carries the native embedded receipt from section 9. Validate its app/framework/platform/runtime/version/content hash against the uploaded declaration, and persist it as typed `embeddedReceipt`, separate from generic metadata. A regular RN OTA upload instead carries `baselineBundleId`, referencing a finalized baseline import in the same app/platform/runtime. Exactly one form is required for an RN bundle; baseline imports cannot point to another baseline. Make the finalized relationship immutable, use a restrictive foreign key for `baselineBundleId`, and enforce agreement in the shared service. Both fields remain null for Capacitor, with no new request requirement. This is archived artifact provenance supplied by an authorized publisher, not server attestation that a store binary was installed. The server still cannot verify encrypted plaintext from its declaration alone.
...
One CLI action publishes the complete prepared collection under one display version, with independent optimistic concurrency and idempotency for every platform/runtime. Upload/finalize all selected artifacts and their baselines, then preflight every variant before publishing any selected lane. Keep a durable receipt of the intended targets and individual results. Resume uncertain operations only within the RN replay rules below; an uncertain result is not evidence that publication failed. Show partial success explicitly. Separate manifest writes cannot promise simultaneous publication, and devices check at different times regardless.

Keep existing Capacitor publish/revert request-hash bytes unchanged: `bundleId` already determines platform/runtime and `releaseId` identifies a revert. Verify supplied targeting before idempotency replay rather than adding redundant targeting fields to legacy hashes. The CLI gives each RN variant a distinct operation key in its receipt. RN publication and reversion use a separately versioned request hash that also binds the new RN intent fields below; select this branch from the stored app framework. Baseline binding is immutable on the bundle, so publication needs no independent baseline override.

### RN publication retries

The inherited mutation service scopes keys to organization, actor and operation, and retains published records for seven days. A lane can return to the same release ID after a revert. Therefore matching `expectedCurrentReleaseId` alone cannot prove that an earlier attempt never committed. Preserve that service's existing Capacitor contract, retention and worker behavior.

For RN, preparation returns `rnIntent: { version: 1, actorKey, preparedAt }`, using the authenticated actor identity and server time. Every RN publish/revert caller, including HTTP, dashboard and MCP, supplies that unchanged intent with the original operation key and expected-current value. Before creating a new mutation, the shared RN service requires the authenticated actor to match and the intent to be no more than 24 hours old; reject malformed/future timestamps and do not extend the window on retry. Use the database clock for preparation and age validation across server instances. This bounded window is deliberately shorter than the current seven-day replay retention. RN must not be enabled on a server whose effective retention is shorter than this window plus a safety margin. These fields describe retry scope, not an authorization credential; normal organization permissions still apply. Automatic RN reverts prepare once under their system actor and preserve that operation on retry; replay workers do not prepare replacement operations.

Persist intent, operation key, exact request arguments and a `not-sent`/`outcome-unknown`/`committed` result state. Durably transition to `outcome-unknown` before sending the request; mark `committed` only after recording its confirmed server result. A crash or lost response leaves an uncertain operation eligible only for replay under the same actor and within the original window. Key rotation, expired intent, missing receipt or uncertain clock evidence stops automatic resubmission with a specific reconciliation diagnostic. Do not generate a new key, refresh the intent, or infer non-publication from the current lane ID automatically. A newly prepared publication after reconciliation is a separate deliberate operation. Never store credentials in the receipt.

An existing matching committed mutation can be read or have its derived manifest reconciled using its original result; this must not create a new release. Authorized reconciliation may inspect the original operation after the automatic retry window, but cannot republish it. Return historical completion separately from the currently active lane, since a completed release may already have been reverted. The existing pending-mutation worker continues synchronizing the lane's current database state. No RN intent fields or new retry errors are imposed on Capacitor requests.
...
Resolve framework from the app record; validate target platform from the request and persist it in the upload session. Do not trust an arbitrary uploaded `framework` to bypass the app's validator. Legacy missing-platform requests are accepted only for Capacitor cross uploads. RN requires an explicit OS, nonempty runtime, `contentHash`, baseline provenance, and a bounded plaintext payload file inventory for both ZIP and deltas. Reuse `UploadSession.files` for that inventory, copy it to a nullable `Bundle.contentFiles` JSON field on finalization, and use the existing canonical file-list hash for `contentHash`. These fields are mandatory for RN and absent for legacy Capacitor; customer-supplied generic metadata cannot override them. Retain the 5,000-file limit; RN uses the additional byte limits below for either strategy. Sections 3 and 9 define the hashes and the embedded copy.

ZIP finalization currently checks object size and records the supplied hash without inspecting a web entrypoint. Preserve the existing transfer service while validating RN's declared inventory and enforcing the new uniqueness key. The CLI validates the actual files before archiving/encryption; native code verifies downloaded/extracted bytes against both signed identities. The server cannot prove an encrypted ZIP's contents from a caller-supplied inventory, and must not describe this preflight as byte verification. In the **RN branch only**, return an existing bundle on finalize conflict only when app, version, platform, runtime, transport hash, content hash, size, strategy, encryption and baseline provenance agree; otherwise return a conflict. Keep Capacitor's existing initiate/finalize conflict responses and validation behavior unchanged, including replay of uploads initiated before migration. Any independent hardening of that existing behavior requires its own compatibility review. [ZIP finalization](../../packages/console/app/api/v1/apps/[appId]/bundles/finalize/route.ts)
...
For RN format 1, limit **each path component to 255 UTF-8 bytes** and the **complete relative path, including separators, to 512 UTF-8 bytes**. Measure accepted spelling before URI percent-encoding; do not substitute JavaScript string length or Swift character count for bytes. Check the actual canonical staging/installed root against the platform's full-path limit, including separators and the terminating byte, before writing files. Keep generated root names bounded and test the full allowed relative-path budget under release-build roots on both OSes. Do not shorten or normalize an accepted payload path at install time. This rejects a DOM filename containing 140 `é` characters plus `.png` (284 bytes), even when macOS can create it. Existing Capacitor character limits remain unchanged. [Ext4 filename storage](https://github.com/torvalds/linux/blob/v6.12/fs/ext4/ext4.h#L2157), [F2FS filename storage](https://github.com/torvalds/linux/blob/v6.12/include/linux/f2fs_fs.h#L219)
...
Before any exporter copies or combines assets, validate the complete source-to-output mapping for the same Metro graph and configuration used by that export, including every scale and the DOM/public output namespaces. Use the installed exporter's destination calculation, then apply the RN byte/path-tree rules. Reject distinct source files that map to the same destination, naming both sources; repeated references to the same source/scale may be deduplicated. For example, Android export maps `assets/icons/log-in.png` and `assets/icons/login.png` to `drawable-mdpi/icons_login.png`. Post-export inventory validation cannot detect the first file after it has been overwritten. A custom exporter that cannot expose or validate its mapping must fail export with a precise unsupported-configuration diagnostic. Do not silently rename files or patch the customer's Metro installation. Verify the resulting output inventory against the validated mapping before hashing, for both embedded builds and OTA exports. This integration is RN-only; Capacitor's exporter remains unchanged.
...
The v3 channel line uses its JSON representation: `channel:null` for the base lane and `channel:"null"` for a channel named `null`. Named channels retain the existing restricted channel alphabet. This distinguishes nullable and named lanes without changing Capacitor v2 canonicalization. Encode canonical lines as UTF-8, separated by a single LF with no trailing newline; preserve the field order above and the existing encryption block encoding. The [literal signature fixtures](../../packages/console/lib/manifest-signing.test.ts) record the implemented bytes for native verifier acceptance.
...
Every RN platform/channel/runtime lane needs a known-good baseline before its first OTA release. The baseline remains an ordinary bundle/release, with the typed import provenance defined in section 2, and the build pipeline makes the embedded copy recognizable without downloading it. The shared RN release service enforces baseline availability for every caller; it is not a CLI-only convention.
...
3. RN upload imports or adopts the archived baseline and finalizes its relationship to the OTA bundle **even when uploading without publishing**. A later dashboard/API/MCP publication can then resolve both artifacts from the server. In an empty lane, the shared release service validates both finalized artifacts, locks the complete lane, checks the reviewed expected-current value, and creates baseline history plus the requested OTA in one database transaction with one RN operation receipt. The baseline release has no auto-revert/force flag inherited from the OTA. Assign strictly ordered `promotedAt` values so the requested OTA sorts after its baseline under the existing lane query; transaction timestamp defaults or random UUID ordering must not decide which is current. Keep subsequent RN publication ordering consistent with those values. Synchronize only the resulting current manifest after commit, with the usual pending-sync repair. No intermediate baseline manifest is required. If the requested artifact is itself the imported baseline, create just one release. Missing/invalid provenance fails before any release write. A competing first publisher changes the expected-current value and produces a conflict; never overwrite it or refresh that expectation automatically. Store builds still require no network publication or preallocated release ID.
...
On an established lane, publishing an OTA uses its reviewed current release normally; its baseline relationship does not reset that lane or insert a new baseline ahead of the existing history. RN revert must reject removing the final remaining baseline release when no prior target exists. App deletion and billing cleanup retain their existing manifest-removal behavior. Capacitor first publication, last-release revert and missing-manifest behavior remain unchanged.

Baseline adoption is a separate RN preparation step, not a relaxed finalize retry. Resolve a finalized imported baseline by its full app/platform/runtime/version key, including a row whose uploader stopped before creating a release. Require matching plaintext `contentHash`, inventory and native receipt identity. Verify the stored representation satisfies the requested strategy and encryption policy: no encrypted-to-plaintext fallback, no automatic key change, and the configured key must be able to read an adopted encrypted ZIP. Different DEKs/nonces under the same permitted key may be accepted. Reuse the stored bundle ID and record its actual transport hash, size and encryption envelope in the receipt before publishing; never pretend the unused ciphertext was uploaded successfully. For an adopted ZIP, the CLI fetches/verifies the stored transport, decrypts when needed, and checks the plaintext inventory against the archived baseline before completing adoption. A missing/corrupt object or conflicting content/policy yields a specific baseline conflict without overwriting the existing artifact. Ordinary OTA/upload retries still require exact transfer identity. Concurrent baseline finalization conflicts return to this explicit adoption step; they do not rewrite receipts implicitly.
...
The export receipt lists each platform/runtime variant, shared release version, native build label(s), transport hash, content hash, private source-map location, baseline receipt, and native build record. Upload records the finalized baseline bundle ID and immutable OTA-to-baseline relationship, including when no release was requested. Multiple CI jobs for supported native environments may write receipts into one export directory; validate that they agree on app/version, contain every configured target, and have at most one artifact for each platform/runtime. Reject conflicting duplicates. The same `upload ... --release production` action publishes this complete set after every artifact and baseline is finalized. It does not rebuild an older native environment from the current checkout or fabricate exports for missing runtimes.
...
Retrying RN upload/release reads the receipt and reuses completed upload IDs, artifact/baseline bundle IDs, and per-variant operation keys and intents. Empty-lane initialization has one atomic publication result, not separately resumable baseline and OTA releases. Check finalized responses against expected app, target, runtime, version, both hashes, transfer settings and baseline provenance before publication. Export once and preserve the exact archive for an ordinary encrypted upload retry; use section 9's explicit, verified baseline adoption when another job already finalized equivalent baseline content. Persist uncertain publication outcomes and follow section 2's actor/window checks before any resend. If publication partly succeeds, retain successful lanes; reconciling a changed or uncertain lane and preparing a new operation requires a deliberate publication decision. Never refresh its expectation/intent automatically or revert successful targets automatically. Capacitor receipts and retry behavior remain unchanged.

`doctor` checks native bootstrap installation, binary-embedded content receipt, baseline archive availability, runtime/source inventory, supported published RN/Expo versions, competing updater ownership and JS imports, trust keys, Metro entry/asset mapping validation, portable filename byte limits, the two Expo adapters, source-map exclusion, and readiness integration. Check the user's existing resolver is still called and that both embedded/OTA exports install the bootstrap. Report background providers that create a second RN host. Static source inspection cannot prove that readiness runs; the release fixture verifies actual behavior.
...
| 3     | Native build records, validated export mappings, grouped publication and server-enforced baselines | One version serves multiple variants; embedded matching incurs zero transfer; atomic initialization, baseline adoption and bounded retries pass |
...
| Two concurrent first publishers propose the same or different baselines                                               | Equal finalized baseline content can be adopted; only one expected-empty publication commits; the other conflicts without a refreshed expectation        |
| RN upload without publication, then first publication through dashboard, HTTP or MCP                                  | Every accepted OTA has a finalized baseline and actual rollback target; missing/mismatched provenance fails before release writes                        |
| Empty-lane initialization interrupted before/after commit; frozen timestamp and reversed UUID ordering                | Baseline and OTA history commit together once; OTA is current; later RN publications sort correctly; only the final manifest is synchronized             |
| RN baseline is the sole remaining release; operator/auto-revert attempts to remove it                                 | No empty lane is created by revert; the baseline remains available; deletion and billing cleanup keep their documented behavior                          |
| Lost publish response, revert B to original A, then retry under same actor, rotated key or expired intent             | Valid same-actor replay reports historical completion without recreating B; scope loss stops resubmission; current A remains authoritative               |
| Crash before persisting a response; missing receipt; RN intent exceeds retention safety budget; delayed sync worker   | Uncertain outcomes never generate replacement operations automatically; unsafe retention prevents RN enablement; worker only repairs current state       |
| Two independent encryptions of one baseline; A finalizes and stops before publishing; B resumes                       | B explicitly verifies/adopts A's finalized bytes and records the stored envelope; content, key-policy or corrupt-object conflicts fail without overwrite |
| Expo Android assets `log-in.png` and `login.png`, flattened directory names, and distinct density variants            | Conflicting source destinations fail before copy/receipt creation; non-colliding assets and supported scales preserve their mapper output                |
| RN components of 254/255/256 UTF-8 bytes; multibyte directories; full paths under actual native staging/install roots | Export, upload and native checks share byte boundaries; 284-byte example fails export; accepted paths fit both native root budgets                       |
| Existing Capacitor initiate/finalize conflicts, old requests/receipts, last-release revert and publication responses  | Existing behavior and response shapes remain intact; no RN intent, baseline requirement, byte-limit rule or new request hash reaches the legacy branch   |
...
Select new validation and operation semantics from the stored app framework, never a caller-controlled opt-in flag. Keep Capacitor request requirements, response/status shapes, defaults, upload conflict behavior, request hashes and replay semantics intact. RN baseline provenance remains null on legacy rows. The stricter RN artifact comparison, publication intents, baseline initialization and UTF-8 byte limits apply only to RN. Any independent hardening of existing Capacitor behavior needs a separate change and compatibility review.
...
1. Expand with framework/target fields, nullable release runtime snapshots, RN content identity, upload inventory and nullable typed baseline provenance. Keep the old global `(appId, version)` constraint while old writers run. Backfill release snapshots from bundles; during mixed deployment, lane reads derive targeting from the bundle so rows inserted by old writers remain readable. Leave Capacitor baseline provenance null. RN creation stays disabled.
2. Deploy all new writers and framework/target/runtime-aware upload/finalize, summary, release, mutation, revert, auto-revert and manifest paths. New release writes copy targeting from bundles. Preserve Capacitor publish/revert request-hash bytes and replay of legacy mutations; normalize absent historical platform to cross. Gate RN intent validation, versioned hashing and atomic baseline initialization on the stored RN framework, with RN creation still disabled. Keep Capacitor's v2 writer unchanged.
...
4. Remove the old global unique constraint only after all readers/writers use complete keys or IDs. Verify the RN intent/retention safety budget and baseline invariants before enabling RN app creation and same display versions across RN variants. RN finalize conflict checks compare the full expected artifact, including content identity and provenance; Capacitor retains existing conflict responses. Keep the cross-only unique index permanently.
...
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Prisma schema/migrations               | App framework; bundle/upload/mutation targets; copied release lanes; RN runtime-aware and legacy cross uniqueness; RN content identity and baseline provenance |
| App APIs                               | Create/list framework; validate bundle targets against that framework                                                                                          |
| Upload/finalize APIs                   | RN target/runtime/content inventory, baseline provenance and strict RN conflict comparison; unchanged Capacitor conflict behavior                              |
| Payload validator                      | Framework-specific entrypoints; RN portable path tree and content identity                                                                                     |
| Release/revert/auto-revert             | Complete lanes throughout; RN intent-bound retries, atomic baseline initialization and final-baseline protection; legacy behavior preserved                    |
...
The later [deep review](deep-review.md) checked inherited service behavior and reproduced exporter/encryption/path counterexamples at repository revision `86acc2eee590204307a89e3c8fa130fe021456da`. Its unchanged probes establish the reasons for this plan revision; they do not implement or validate the proposed fixes. The expanded section 13 acceptance cases must pass when the RN implementation is built. Those review corrections were planning-only; subsequent production changes and database tests are recorded in [implementation status](implementation-status.md).
