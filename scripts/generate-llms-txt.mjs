...
const MCP_DOCS_OUTPUT = path.join(ROOT, 'packages/mcp-core/src/generated-docs.ts');
...
async function main() {
  ...
  const generatedMcpDocsSource =
    `// Generated by scripts/generate-llms-txt.mjs. Do not edit manually.\n` +
    `import type { DocumentationPage } from './docs.js';\n\n` +
    `export const GENERATED_DOCUMENTATION = ${JSON.stringify(
  ...
  const prettierConfig = (await resolveConfig(MCP_DOCS_OUTPUT)) ?? {};
  const generatedMcpDocs = await format(generatedMcpDocsSource, {
    ...prettierConfig,
    filepath: MCP_DOCS_OUTPUT,
  });

  for (const target of OUTPUTS) await emit(target, output);
  await emit(MCP_DOCS_OUTPUT, generatedMcpDocs);
}
