...
const featureRows = [
  ...
  ['Native-compat guardrail', 'Yes — CLI checks at upload time', 'No equivalent'],
  ...
  ['Open source', 'Entire stack, MIT — plugin, CLI, dashboard, server', 'Plugin only; cloud is closed'],
];
...
export default function CapawesomeAlternativePage() {
  return (
    <BlogArticle post={post}>
      ...
      <p>
        Capawesome&apos;s live-update plans are gated by monthly active users: as of July 2026,
        $9/mo covers just 1,000 MAU, then $29 (10K), $79 (50K), and $249 (250K). Every active
        device counts every month, whether you release or not — and 1,000 MAU is small enough that
      ...
      <p>
        ...
        business, you take the stack and go. That exit right is the difference between a vendor
        and a dependency.
      </p>

      <h2>Stronger delivery, stricter safety</h2>
      <p>
        OtaKit devices download manifests and bundles exclusively from Cloudflare&apos;s global
        edge — vendor servers are never in the device path, so delivery reliability and latency
        are the CDN&apos;s, worldwide. Security is layered and always on: every manifest is
        ES256-signed, every download SHA-256-verified before it runs, and every activation is
        provisional until your app calls <Code>notifyAppReady()</Code> — no confirmation, the
        device rolls itself back. For code-sensitive apps, opt-in end-to-end encryption
        (AES-256-GCM, your key) means even OtaKit can&apos;t read your bundles.
      </p>
      <p>
        And one thing no one else in the category does: the OtaKit CLI inspects your dependencies
        at upload time and warns when a bundle depends on native code the installed app
        doesn&apos;t have — catching the classic OTA crash before it ships instead of after.
      </p>
...
otakit upload --release`}</Pre>
      <p>
        ...
        <A href="/docs/setup">ten-minute setup</A> on a test app first and watch an update land.
      </p>
    </BlogArticle>
  );
}
