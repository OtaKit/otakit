  ...
  return (
    <BlogArticle post={post}>
      ...
      <p>
        ...
        remotely; OtaKit handles getting the result onto your device. This post wires them
        together for a Capacitor app.
      </p>
      ...
      <ul>
        <li>
          ...
          Claude iOS app and on the web (claude.ai/code): sessions execute in Anthropic&apos;s
          cloud sandbox against your GitHub repo, and your phone is the steering wheel. If you
          prefer your own hardware, remote control mode can drive a Claude Code CLI running on your
          desktop instead.
        </li>
        <li>
        ...
        <li>
          <strong>A CI step that uploads on push</strong> — the piece that turns &ldquo;merged&rdquo;
          into &ldquo;on my phone.&rdquo;
        </li>
      </ul>

      <h2>The division of labor</h2>
      <p>
        The workflow works when each side does what it&apos;s good at. The phone is good at
        intent, review, and verification: describing the change, reading the diff, tapping through
        the result. The cloud (or your remote machine) is good at repo access, dependency installs,
...
otakit upload --release staging`}</Pre>
      ...
      <p>
        That last step is the payoff: the phone that wrote the change is the phone that verifies
        it, on a real device, inside the real native shell.
      </p>
      ...
      <Callout>
        <p>
          ...
          debug menu — then your personal install rides the experimental lane while every other
          user stays safely on production.
        </p>
      </Callout>

      <h2>Why this is safe to do at all</h2>
      <p>
        Shipping code you reviewed on a phone screen sounds reckless until you remember what the
        OTA layer is doing underneath. Every bundle is verified against a signed manifest before it
        runs. Every activation is provisional until the app calls <Code>notifyAppReady()</Code> —
        if your experiment white-screens on boot, the device rolls itself back to the last good
        bundle automatically. And the blast radius is whatever the <Code>staging</Code> channel
        contains: your device, not your users.
      </p>
      <p>
        The OtaKit CLI also runs a native-compatibility check on every upload, so if Claude
        helpfully added a Capacitor plugin that needs native code your installed shell
        doesn&apos;t have, you find out at upload time — not from a crash.
      </p>
      ...
      <ul>
        <li>
          ...
          real build and a store release. The compatibility guardrail will tell you when
      ...
      <p>
        ...
        each becomes a window where the app can actually move forward, with the same rollback
        safety net as any other release.
      </p>
      <p>
        ...
        <A href="/docs/ci">CI automation guide</A>.
      </p>
    </BlogArticle>
  );
}
