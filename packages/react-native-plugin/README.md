...
The iOS host observes application lifecycle changes before asynchronous storage preparation. If preparation finishes in the background, it retains staged code for a later eligible activation. Inactive/background time does not consume readiness time, including transitions before the RN host completion callback. UI intent remains separate from current activity state, so a temporary interruption does not turn a normal UI launch into a permanently deferred headless host.
