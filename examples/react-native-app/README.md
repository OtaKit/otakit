...
On a memory-constrained development machine, finish Gradle before starting the emulator. The API 36 AOSP `system-images;android-36;default;arm64-v8a` image passed the full bare RN and Expo suites with two cores, 1536 MiB RAM and a 720×1280 display. Wait for `adb -s emulator-5554 shell getprop sys.boot_completed` to return `1` before running tests. Run one platform at a time; retain the matching APK/app, exports, receipts and logs before removing temporary build caches and the test AVD. An earlier Google APIs image caused system-wide ANRs on the 8 GiB test machine; app readiness deadlines should not be increased to accommodate an unresponsive emulator.

To exercise the optional installed Android hook instead, supply an Android native-input JSON that records its host settings file, then build with `-PotakitNativeInputs=/absolute/path/to/inputs.json`. `-PotakitEmbeddedVersion=embedded` optionally selects the embedded display version. This mode excludes the manually prepared OtaKit asset folder and lets the hook export, stage, package and seal fresh resources. It prints the archive directory under `android/app/build/otakit/release/`. The generated input record adds actual Gradle variant/dependency evidence; use that record for subsequent OTA export. The manual OTA scenario generator uses independent test keys, so its earlier manifests do not automatically match a hook-built runtime.

The iOS fixture also opts into the installed hook when invoked through `otakit rn build-ios --project examples/react-native-app --workspace ios/HelloWorld.xcworkspace --scheme HelloWorld --sdk iphonesimulator --destination "platform=iOS Simulator,id=<UDID>" --derived-data /private/tmp/otakit-fixture-derived-data --native-inputs /absolute/path/to/ios-inputs.json --version embedded --output /private/tmp/otakit-fixture-build-1 --no-code-signing`. Run this example from the repository root after building the CLI. Its native-input file must record the host settings. Use a new output directory for each build and preserve its completed receipt, resolved inputs and embedded export for OTA. Ordinary Xcode builds retain the existing manual fixture path. See the [hook setup](../../packages/react-native-plugin/README.md#ios-xcode-build-hook).
